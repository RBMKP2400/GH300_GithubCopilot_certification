...
def gen_copilot_instructions(lang, coverage):
    extra_by_lang = {
        "python": (
            "- Evita print() en produccion; usa logging\n"
            "- Prefiere f-strings sobre .format() o %\n"
            "- Longitud maxima de funcion: ~40 lineas"
        ),
        "node": (
            "- Evita any en TypeScript\n"
            "- Prefiere const sobre let, nunca var\n"
            "- Async/await sobre callbacks y .then()"
        ),
        "java": (
            "- Prefiere Optional<T> sobre retornar null\n"
            "- Usa interfaces para inyeccion de dependencias\n"
            "- Anota tests con @Test, @BeforeEach, @AfterEach"
        ),
    }
    meta = {
        "python": {
            "lint":      "flake8 (max-line-length=88, flake8-bugbear)",
            "dead":      "vulture (min-confidence 80)",
            "security":  "bandit (-ll)",
            "test_fw":   "pytest",
            "test_desc": "pytest + pytest-cov (umbral >= {}%)".format(coverage),
            "style":     "snake_case para funciones/variables, PascalCase para clases",
            "imports":   "stdlib -> third-party -> local (isort)",
            "types":     "type hints en todas las funciones publicas",
        },
        "node": {
            "lint":      "ESLint (--max-warnings=0)",
            "dead":      "ts-unused-exports",
            "security":  "npm audit (--audit-level=high)",
            "test_fw":   "Jest",
            "test_desc": "Jest + coverage (umbral >= {}%)".format(coverage),
            "style":     "camelCase para vars/funciones, PascalCase para clases/componentes",
            "imports":   "ES Modules (import/export), no CommonJS (require)",
            "types":     "TypeScript strict: true en tsconfig",
        },
        "java": {
            "lint":      "Checkstyle (via Maven)",
            "dead":      "PMD (via Maven)",
            "security":  "OWASP Dependency-Check (via Maven)",
            "test_fw":   "JUnit 5",
            "test_desc": "JUnit 5 + JaCoCo (umbral >= {}%)".format(coverage),
            "style":     "camelCase para metodos/variables, PascalCase para clases",
            "imports":   "Sin wildcard imports; importa clases explicitamente",
            "types":     "Genericos correctamente; evita raw types",
        },
    }
    t = meta[lang]
    extra = extra_by_lang[lang]
    p = []
    p.append("# GitHub Copilot Instructions\n\n")
    p.append("Este archivo configura GitHub Copilot Chat para este repositorio.\n")
    p.append("Copilot leera estas instrucciones en cada conversacion dentro del proyecto.\n\n")
    p.append("## Contexto del proyecto\n\n")
    p.append("- **Lenguaje principal**: {}\n".format(lang))
    p.append("- **Cobertura minima de tests**: {}%\n".format(coverage))
    p.append("- **CI/CD**: GitHub Actions (ci.yml, matrix.yml, pr-checks.yml)\n")
    p.append("- **Rama principal**: `main` (protegida, requiere PR + checks verdes)\n\n")
    p.append("## Estilo de codigo\n\n")
    p.append("- **Nomenclatura**: {}\n".format(t["style"]))
    p.append("- **Imports**: {}\n".format(t["imports"]))
    p.append("- **Tipos**: {}\n".format(t["types"]))
    p.append(extra + "\n\n")
    p.append("## Herramientas del pipeline CI\n\n")
    p.append("| Fase          | Herramienta                              |\n")
    p.append("|---------------|------------------------------------------|\n")
    p.append("| Lint          | {} |\n".format(t["lint"]))
    p.append("| Codigo muerto | {} |\n".format(t["dead"]))
    p.append("| Seguridad     | {} |\n".format(t["security"]))
    p.append("| Tests         | {} |\n\n".format(t["test_desc"]))
    p.append("Cuando generes codigo, asegurate de que pasara estas herramientas sin warnings.\n\n")
    p.append("## Tests\n\n")
    p.append("- Escribe tests unitarios para **toda funcion publica**.\n")
    p.append("- Patron **Arrange / Act / Assert** con secciones comentadas.\n")
    p.append("- Nombre del test: test_should_ACCION_when_CONDICION.\n")
    p.append("- No uses numeros magicos; define constantes con nombre descriptivo.\n")
    p.append("- Cobertura minima: **{}%** (el CI fallara si no se alcanza).\n\n".format(coverage))
    p.append("## Seguridad\n\n")
    p.append("- Nunca incluyas credenciales, tokens, API keys ni contrasenas en el codigo.\n")
    p.append("- Usa variables de entorno o GitHub Secrets para datos sensibles.\n")
    p.append("- Evita funciones inseguras: eval, exec sin validacion, deserializacion sin schema.\n")
    p.append("- TruffleHog escanea el diff de cada PR; cualquier secreto detectado bloqueara el merge.\n\n")
    p.append("## Commits y Pull Requests\n\n")
    p.append("Sigue Conventional Commits estrictamente (el CI valida el titulo del PR):\n\n")
    p.append("  tipo(scope): descripcion en imperativo\n")
    p.append("  Tipos validos: feat | fix | docs | style | refactor | test | chore | ci\n\n")
    p.append("Ejemplos:\n")
    p.append("  feat(auth): add JWT refresh token endpoint\n")
    p.append("  fix: handle null pointer in user service\n")
    p.append("  test: add coverage for edge cases in parser\n\n")
    p.append("## Instrucciones para Copilot Chat\n\n")
    p.append("- Cuando se te pida generar tests, usa {} y sigue el patron AAA.\n".format(t["test_fw"]))
    p.append("- Cuando se te pida revisar codigo, comprueba lint, seguridad y cobertura.\n")
    p.append("- Cuando se te pida crear un workflow, sigue ci.yml con jobs paralelos + needs.\n")
    p.append("- Cuando se te pida documentar, usa docstrings en el formato estandar del lenguaje.\n")
    p.append("- Ante cualquier duda sobre convenciones, consulta este archivo primero.\n")
    return "".join(p)
...
def main():
    ...
    write_file(f"{base}/copilot-instructions.md",                 gen_copilot_instructions(lang, coverage))
  ...
  3. Revisa .github/copilot-instructions.md y personaliza el contexto
  4. Haz commit:
       git add .github && git commit -m 'ci: add github structure'
  5. Haz push y revisa la pestana Actions en GitHub
